...
#     token: str = Depends(get_current_user)
    ...
    try:
        ...
        access_token = "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"
    ...
    try:
        ...
        async with aiomysql.create_pool(**database_config) as pool:
            async with pool.acquire() as conn:
                ...
                # Deserialize the commit_messages string into a Python list
                commit_messages = json.loads(commit_messages[0])
                ...
                for task_number in range(1, 6):
                    ...
                    # Get the specific commit message from the corresponding column
                    task_commit_message = None
                    query = f"""
                        SELECT {task_commit_message_column}
                        FROM {table_name}
                        WHERE project_id = '{project_id}';
                    """
                    async with conn.cursor() as cursor:
                        await cursor.execute(query)
                        task_commit_message = await cursor.fetchone()

                    # Check if the task_commit_message is present in the commit_messages list
                    if task_commit_message and task_commit_message[0] in commit_messages:
                        task_status_column = f"task{task_number}_status"
                        task_status_value = "completed"
                    else:
                        task_status_column = f"task{task_number}_status"
                        task_status_value = "not completed"
...
async def check_and_add_user_type_column(conn, table_name):
    try:
        async with conn.cursor() as cursor:
            # Check if the 'user_type' column exists in the table
            check_query = f"SHOW COLUMNS FROM {table_name} LIKE %s"
            await cursor.execute(check_query, ('user_type',))
            result = await cursor.fetchone()

            if not result:
                # If 'user_type' column doesn't exist, add it to the table
                alter_query = f"ALTER TABLE {table_name} ADD COLUMN user_type VARCHAR(255)"
                await cursor.execute(alter_query)
        await conn.commit()
    except Exception as e:
        raise HTTPException(
...
async def insert_comment(
    ...
    print(course_name, team_id, comment, user_type)
    try:
        ...
        async with aiomysql.create_pool(**database_config) as pool:
            async with pool.acquire() as conn:
                # Form the table name based on the course_name
                table_name = f"{course_name}__comments"

                # Check and add 'user_type' column if not exists
                await check_and_add_user_type_column(conn, table_name)

                # Insert data into the specified comments table
                query = f"""
                    INSERT INTO {table_name} (team_id, comment, timestamp, user_type)
                    VALUES (%s, %s, %s, %s);
                """
                values = (team_id, comment, timestamp_str, user_type)

                async with conn.cursor() as cursor:
                    await cursor.execute(query, values)
                    await conn.commit()

        return JSONResponse(content={"message": f"Comment inserted successfully"}, status_code=200)
